				   ...
				   success:function(data){
                    var g=JSON.parse(data);
					  ...
					  $("*").removeClass("has-error");
						...
						//$(form).submit();
						var t=g.loanid;
						window.location="loansuccess.php?loanid="+t;
						return;
						}
...
<form  id="loanapp" action="<?php echo htmlspecialchars("loanAppController.php?action=create");?>"  role="form" method="post" enctype="multipart/form-data">
...
<span class="col-sm-5">
<textarea class="form-control" id="loan_comment" name="loan_comment" maxlength="1000"  rows="7" type="textarea">
<?php echo $_SESSION['loan_comment'];?>
</textarea>
...
<script>
 $('#characterLeft').text('500 characters left');
    $('#loan_comment').keydown(function () {
        var max = 500;
        var len = $(this).val().length;
        if (len >= max) {
            $('#characterLeft').text('You have reached the limit');
            $('#characterLeft').addClass('error');
            $('#loansubmission').attr('disabled',true);            
        } 
        else {
            var ch = max - len;
            $('#characterLeft').text(ch + ' characters left');
            $('#loansubmission').attr('disabled',false);
...
<li> Copy of your Drivers License</li>
  ...
  <div class="form-group"> 
    <div class="col-sm-offset-2 col-sm-10">
      <button id="loansubmission" type="submit" class="btn btn-default">Submit</button>
    </div>
  </div>
  ...
  <div class="modal fade" id="loanErrorModal" role="dialog">
    <div class="modal-dialog modal-sm" style="margin-top:20%;">
      <div class="modal-content">
         <div class="modal-body text-center" style="">
		 <span class="text-center">
         <p>We do apologise. Your application could not be submitted. Admin was notified. Please try again.</p>
        </span>
		</div>
        </div>
    </div>
  ...
  <div class="modal fade" id="normalErrorModal" role="dialog">
    <div class="modal-dialog modal-sm" style="margin-top:20%;">
      <div class="modal-content">
         <div class="modal-body text-center" style="">
		 <span class="text-center">
         <p>You have one or more errors on the application form.</p>
        </span>
		</div>
        </div>
    </div>
