...
$mail->Body    = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Forshorelending account creation</title></head><body style="margin:0px; font-family:Tahoma, Geneva, sans-serif;"><div style="padding:10px; background:#333; font-size:24px; color:#CCC;"><a href="http://www.forshorelending.com">forshorelending</a>Forshorelending account craeton</div><div style="padding:24px; font-size:17px;">Hello '.$name.',<br /><br />Click the link below to reset your password (link valid for one(1) hour):<br /><br /><a href="http://localhost/forshore/reg_login.php">Click here to login with your loanid and temporary password</a><br /><br />Login after successful password change using your loanid and new password:<br />* loanid: <b>'.$id.'</b><br>Password:<b>'.$enc.'</b></div></body></html>';
...
function findUsers($key){
		...
		try {
    		...
    		// set the PDO error mode to exception
  			$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$stmt=$conn->prepare("SELECT * from register WHERE firstname LIKE '$key%' OR lastname LIKE '$key%'");
			$stmt->execute();
			$res=$stmt->fetchAll(PDO::FETCH_ASSOC);
			if($res){
